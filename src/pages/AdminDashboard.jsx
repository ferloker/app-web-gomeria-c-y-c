...
export default function AdminDashboard() {
  ...
  const { data: inventory, loading: invLoading, setData: setInventory, refetch: refetchInventory } = useGitData('src/data/inventory.json', []);
  const { data: settings, loading: setLoading, setData: setSettings, refetch: refetchSettings } = useGitData('src/data/settings.json', { is_open: true, pause_auxilio: false });
              ...
              <section>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={refetchInventory} className="bg-slate-800 hover:bg-slate-700 active:scale-95 text-slate-300 p-1.5 rounded-lg transition-all border border-white/5" title="Forzar Sincronización con GitHub">
                      <span className={`material-symbols-outlined text-[18px] ${invLoading ? 'animate-spin text-primary' : ''}`}>sync</span>
                    </button>
                  </div>
                </div>
             ...
             <div className="bg-slate-900 rounded-3xl p-6 space-y-6 border border-white/5 shadow-xl">
               ...
               <div className="space-y-4">
                 ...
                 <div>
                   ...
                   <label className="text-xs font-bold text-slate-400 block mb-1.5 ml-1">Coordenadas GPS del Mapa (Lat, Lng)</label>
                   <div className="flex bg-background-dark border border-white/5 rounded-xl overflow-hidden focus-within:border-primary transition-colors shadow-inner mb-4">
                     <span className="flex items-center justify-center px-4 bg-slate-800 text-slate-400"><span className="material-symbols-outlined text-[18px]">location_on</span></span>
                     <input value={settings?.map_coords || ''} onChange={e=>handleChangeSetting('map_coords', e.target.value)} className="w-full p-4 text-sm text-white outline-none bg-transparent font-mono" placeholder="-24.661366, -56.442317" />
                   </div>
                 </div>
               </div>
